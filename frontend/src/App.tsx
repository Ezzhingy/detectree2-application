...
function App() {
  const [environment, setEnvironment] = useState<"Default" | "Forest" | "Urban">("Default");
  const [selectedImage, setSelectedImage] = useState<File | null>(null);
  const [preview, setPreview] = useState<string | null>(null);
  const [confidence, setConfidence] = useState(0.5);
  const [analysisComplete, setAnalysisComplete] = useState(false);
  const [statistics, setStatistics] = useState<{ total_trees: number } | null>(null);
  ...
  const handleDownload = async (fileType: 'png' | 'gpkg') => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/download/${fileType}`);
      if (!response.ok) {
                  ...
                  <div className="flex gap-8 items-center">
                    ...
                    {statistics && (
                      <div className="bg-white p-6 rounded-lg shadow-lg h-fit">
                        <h2 className="text-xl text-black font-bold mb-4">Analysis Results</h2>
                        <table className="min-w-[200px]">
                          <tbody>
                            <tr className="border-b">
                              <td className="text-black py-2 pr-4 font-semibold">Total Trees Detected</td>
                              <td className="text-black py-2 px-4">{statistics.total_trees}</td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
